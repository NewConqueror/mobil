...
Future<void> alarmCallback(int alarmId) async {
  ...
  try {
    ...
    if (now.hour >= 8 && now.hour <= 23) {
      // Random todo reminder messages
      final List<String> todoMessages = [
        'Yapılacaklar listeni kontrol etme zamanı! ✅',
        'Hangi görevini tamamlayacaksın? 📝',
        'Bugün ne kadar ilerleme kaydedtin? 🚀',
        'Hedeflerine bir adım daha yaklaş! 🎯',
        'Yapılmayacaklar listeni de gözden geçir! ❌',
        'Odaklan ve üretken ol! 💪',
        'Küçük adımlar büyük başarılar yaratır! ⭐',
        'Zamanını değerli kullan! ⏰',
      ];

      final randomMessage = todoMessages[now.millisecond % todoMessages.length];
...
Future<void> _showBackgroundNotification({
  ...
  const AndroidNotificationDetails androidDetails = AndroidNotificationDetails(
    ...
    'Todo Hatırlatmaları',
    channelDescription: 'Yapılacaklar listesi hatırlatmaları',
...
class NotificationService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _isInitialized = true;
      print('✅ Todo notification service initialized with AlarmManager');
  ...
  Future<void> _createNotificationChannel() async {
    const AndroidNotificationChannel mainChannel = AndroidNotificationChannel(
      'todo_reminder_channel',
      'Todo Hatırlatmaları',
      description: 'Yapılacaklar listesi hatırlatmaları',
      importance: Importance.max,
      playSound: true,
      enableVibration: true,
      enableLights: true,
    ...
    const AndroidNotificationChannel highPriorityChannel =
        AndroidNotificationChannel(
          'todo_high_priority',
          'Todo Yüksek Öncelik',
          description: 'Önemli todo hatırlatmaları',
          importance: Importance.max,
          playSound: true,
          enableVibration: true,
          enableLights: true,
    ...
    const AndroidNotificationChannel backgroundChannel =
        AndroidNotificationChannel(
          'todo_background_service',
          'Todo Arka Plan Servisi',
          description: 'Todo takibi arka plan servisi bildirimleri',
    ...
    for (int hour = 8; hour <= 23; hour++) {
      ...
      const AndroidNotificationDetails androidDetails =
          AndroidNotificationDetails(
            'todo_high_priority',
            'Todo Yüksek Öncelik',
            channelDescription: 'Önemli todo hatırlatmaları',
            importance: Importance.max,
            priority: Priority.max,
            showWhen: true,
            enableVibration: true,
      ...
      await flutterLocalNotificationsPlugin.zonedSchedule(
        ...
        '📝 Todo Kontrolü',
        'Yapılacaklar listeni kontrol et! Bugün tamamlaman gereken görevler var! ✅',
        scheduledDate,
        notificationDetails,
  ...
  Future<void> _showPersistentNotification() async {
    const AndroidNotificationDetails androidDetails =
        AndroidNotificationDetails(
          'todo_background_service',
          'Todo Arka Plan Servisi',
          channelDescription: 'Todo takibi arka planda çalışıyor',
          importance: Importance.low,
